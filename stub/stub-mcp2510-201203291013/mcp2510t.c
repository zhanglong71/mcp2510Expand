...
/** Clear interrupt flag **/
        ...
        {
            printk("[%s-%d]Warn: ... RcvFrm.Messages[0] = 0x%x received self data or error !!! \n", __func__, __LINE__, pRcvMsg->stRcvCanFrm.Messages[0]);
       ...
       if(NULL != (node_rb = RcvCanFrm_complete(&rcvDataRoot_rb)))
        {
            ...
            RcvCanFrm_reset(&rcvDataRoot_rb);
        }
        ...
        else if (wait_event_interruptible(wqCanBuff, RcvCanFrm_complete(&rcvDataRoot_rb)))
        {
