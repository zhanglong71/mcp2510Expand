 ...
 *
 * eg. all off: ./app_raw /dev/mcp510 010f0e00,00000000,0000,4201000100     
 **/
...
int main(int argc,char *argv[])
{
    ...
    if(argc >= 3)
    {
        ...
        for(i = 0; i < iLen; i++)
        {
            ...
            {
                printf("[%s-%d]Warning: data(argv[2][%d] = %d) argument error!\n", __FILE__, __LINE__, i, (int)argv[2][i]);
                goto err1;
            }
        }
        ...
        if(argc >= 4)
        {
            ...
            {
                if((argv[3][0] >= '0') && (argv[3][0] <= '9'))  /** 0-9 **/
                {
                    adapterNo = argv[3][0] - '0';
                }
                else if((argv[3][0] >= 'A') && (argv[3][0] <= 'F')) /** A-F **/
                {
                ...
                {
                    printf("[%s-%d]Warning: adapterNo(argv[3][0]) argument invalid!\n", __FILE__, __LINE__);
                    goto err1;
                }
            }

            if(adapterNo != (pSendCmd->cmd[2] & 0x0f))
            {
                printf("[%s-%d]Warning: adapterNo(argv[3][0]) argument error!\n", __FILE__, __LINE__);
            }
        } else if(argc >= 3) {
            adapterNo = pSendCmd->cmd[2] & 0x0f;
        }
    }
    else
    {
