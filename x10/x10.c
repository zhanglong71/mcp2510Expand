...
dev_t   devno;
#define MTypeNo_X10 0x02    /** 类型编号(2 - x10) **/
...
static ssize_t x10Device_write(struct file *filp, const char __user *buf, size_t size, loff_t *ppos)
{
    int iRet = 0;
    unsigned short usTmp;
    struct sendCmd_s *pSendCmd;
...
static struct file_operations x10Device_fops = {
    .owner = THIS_MODULE,
    .read = x10Device_read,
    .write = x10Device_write,
    .ioctl = x10Device_ioctl,
    .open = x10Device_open,
    .release = x10Device_release,
};
...
static int  __init x10Device_init(void)
{
    ...
    ret = alloc_chrdev_region(&devno, 0, MMaxMinorNum, "x10");
    if (ret) {
        printk("[%s-%d]Warn: Alloc device number failed.\n", __func__, __LINE__);
        return ret;
    }

    register_canDev(devno, MTypeNo_X10, &x10Device_fops);
